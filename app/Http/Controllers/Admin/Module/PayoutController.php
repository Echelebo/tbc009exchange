...
class PayoutController extends Controller
{
    ...
    public function payoutList(Request $request)
    {
        ...
        $data['payouts'] = collect(PayoutRequest::selectRaw('COUNT(id) AS totalPayout')
            ->selectRaw('COUNT(CASE WHEN status = 2 THEN id END) AS pendingPayout')
            ->selectRaw('(COUNT(CASE WHEN status = 2 THEN id END) / COUNT(id)) * 100 AS pendingPayoutPercentage')
            ->selectRaw('COUNT(CASE WHEN status = 3 THEN id END) AS completePayout')
            ->selectRaw('(COUNT(CASE WHEN status = 3 THEN id END) / COUNT(id)) * 100 AS completePayoutPercentage')
            ->selectRaw('COUNT(CASE WHEN status = 5 THEN id END) AS cancelPayout')
            ->selectRaw('(COUNT(CASE WHEN status = 5 THEN id END) / COUNT(id)) * 100 AS cancelPayoutPercentage')
            ->selectRaw('COUNT(CASE WHEN status = 6 THEN id END) AS refundPayout')
            ->selectRaw('(COUNT(CASE WHEN status = 6 THEN id END) / COUNT(id)) * 100 AS refundPayoutPercentage')
            ->get()
            ->toArray())->collapse();
    ...
    public function payoutListSearch(Request $request)
    {
        ...
        $payouts = PayoutRequest::with(['user:id,firstname,lastname,username,image,image_driver'])
            ...
            ->when(isset($payoutType), function ($query) use ($payoutType) {
                ...
                } elseif ($payoutType == 'refund') {
                    return $query->where('status', 6);
                } else {
                    return $query->whereIn('status', ['0', '1', '5', '6']);
                }
            })
        ...
        return DataTables::of($sells)
            ...
            ->addColumn('send_amount', function ($item) {s
