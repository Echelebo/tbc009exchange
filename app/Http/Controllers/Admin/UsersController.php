...
class UsersController extends Controller
{
    ...
    public function referralSearch(Request $request)
    {
        ...
        $referrer = User::with('referredUsers', 'referrer')->orderBy('id', 'DESC')
            ->when(!empty($search), function ($query) use ($search) {
                return $query->where(function ($subquery) use ($search) {
    ...
    public function topupSearch(Request $request)
    {
        ...
        $topup = TopUpRequest::with('user')->orderBy('id', 'DESC')
            ->when(!empty($search), function ($query) use ($search) {
                return $query->where(function ($subquery) use ($search) {
                    $subquery->where('username', 'LIKE', "%$search%")
                    ->orWhere('remarks', 'LIKE', "%{$search}%")
        ...
        return DataTables::of($topup)
            ...
            ->rawColumns(['user', 'method', 'amount', 'status', 'action',])
            ->make(true);
    }

    public function topupAction(Request $request, $id)
    {
    ...
    public function payoutSearch(Request $request)
    {
        $search = $request->search['value'] ?? null;
        $filterTransactionId = $request->filterTransactionID;
        $filterDate = explode('-', $request->filterDate);
        ...
        $payout = PayoutRequest::with('user')->orderBy('id', 'DESC')
            ->when(!empty($search), function ($query) use ($search) {
                return $query->where(function ($subquery) use ($search) {
                    $subquery->where('username', 'LIKE', "%$search%")
                    ->orWhere('remarks', 'LIKE', "%{$search}%")
    ...
    public function payoutAction(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            'id' => 'required',
            'status' => ['required', Rule::in(['1', '3'])],
